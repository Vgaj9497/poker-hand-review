...
**poker-hand-review** 讀取 Natural8 / GGPoker 錦標賽匯出的手牌歷史，從 **你本人（Hero）** 的視角，把每個決策標成 🟢 可接受 / 🟡 不準 / 🔴 失誤，並附上 GTO 建議與理由。看完一場，你會清楚知道「我哪幾手打錯、錯在哪、該怎麼打」。
...
> 還沒有自己的手牌？repo 內附合成範例 `data/sample.txt`，可直接試跑：`poker-hand-review analyze data/sample.txt`。
...
> 下列截圖帶你快速看懂 Web UI(最上方主圖是完整介面總覽;點任一張可放大)。

<table>
<tr>
<td width="50%" align="center"><b>① 手牌列表 Hand list</b><br><sub>逐手 ID／底牌／位置／淨利,以 🟢🟡🔴 標示失誤等級</sub><br><img src="../screenshots/hand-list.png" alt="Hand list" width="210"></td>
<td width="50%" align="center"><b>② 逐手回放 Hand replay</b><br><sub>牌桌＋動作時間軸＋決策評分卡(GTO／solver 建議)</sub><br><img src="../screenshots/hand-replay.png" alt="Hand replay" width="360"></td>
</tr>
<tr>
<td width="50%" align="center"><b>③ 漏洞 Leaks</b><br><sub>重複失誤模式:次數、累計 EV 損失、對應手牌</sub><br><img src="../screenshots/leaks.png" alt="Leaks" width="270"></td>
<td width="50%" align="center"><b>④ 各位置盈虧 Positions</b><br><sub>各位置淨輸贏,看哪個位置在漏錢</sub><br><img src="../screenshots/positions.png" alt="Net by position" width="300"></td>
</tr>
</table>
...
| 指令 | 用途 |
|---|---|
| `poker-hand-review analyze <路徑>` | 逐手彩色檢討 + 統計 + 漏洞；加 `--json report.json` 匯出給 Web UI |
...
| `poker-hand-review web --report report.json` | 啟動 Web UI 本機 server |

**常用選項**

```powershell
poker-hand-review analyze ".\data" --hero "Hero"          # 指定 Hero 名稱（預設 "Hero"）
poker-hand-review analyze ".\data" --min-tier inaccuracy  # 只顯示不準以上的手
...
Solver adapter 的 JSON 契約見 [`docs/SOLVER_ADAPTER.md`](../SOLVER_ADAPTER.md)。
